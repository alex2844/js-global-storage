...
let updateImgId = () => {
	...
	sheet.getSheets().forEach(list => {
		list.getRange('H1:I10000').clear();
		for (let i=0; i<list.getDataRange().getNumRows(); ++i) {
			let v = list.getRange('G'+(i+1)).getValue();
			list.getRange('H'+(i+1)).setValue((i == 0) ? 'imgId' : ((files[v] && files[v].id) || null));
			list.getRange('I'+(i+1)).setValue((i == 0) ? 'imgUrl' : ((files[v] && files[v].url) || null));
			delete files[v];
		}
	});
